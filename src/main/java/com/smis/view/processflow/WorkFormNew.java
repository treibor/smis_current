...
public class WorkFormNew extends VerticalLayout {
	/**
	 * 
	 */
	...
	private InstallmentNew installment;
	Binder<WorkNew> binder = new BeanValidationBinder<>(WorkNew.class);
	...
	public WorkFormNew(NewService service) {
	...
	public void confirmDelete(WorkNew work) {
		...
		if (work == null) {
			...
			Notification.show("Sanction No: Special Characters like *, ?, ^,%, $ ,#  are not allowed.", 5000, Position.TOP_CENTER).addThemeVariants(NotificationVariant.LUMO_ERROR);
				...
				try {
				    ...
				    // Retrieve and save the first SchemeProcess
				    /*
				    SchemeProcess firstProcess = service.getSchemeProcessByScheme(work.getScheme());
				    //System.out.println(firstProcess.getScheme().getSchemeName());
				    if (firstProcess != null) {
				        ProcessFlow processFlow = new ProcessFlow();
				        processFlow.setSchemeprocess(firstProcess);
				        processFlow.setWork(work);
				        //processFlow.setIsComplete(false);
				        processFlow.setAssignedDate(LocalDateTime.now());
				        processFlow.setUser(service.getLoggedUser()); // Assign current user
				        service.saveProcessFlow(processFlow);
				    }
				    */
				    Notification.show("Entered Successfully", 
		                      5000, Position.TOP_CENTER)
		               .addThemeVariants(NotificationVariant.LUMO_SUCCESS);
				} catch (Exception e) {
				    Notification.show("Unable to Save Work. Please ensure all mandatory fields are entered.", 
				                      5000, Position.TOP_CENTER)
	...
	private void SaveInstallments() {
		...
		if (work == null) {
			...
			try {
				InstallmentNew installment = new InstallmentNew();
	...
	public BigDecimal calculateReleasedInstAmount(WorkNew work) {
	...
	public static abstract class WorkFormEvent extends ComponentEvent<WorkFormNew> {
		private WorkNew work;

		protected WorkFormEvent(WorkFormNew source, WorkNew work) {
			super(source, false);
			this.work = work;
		}
	...
	public static class SaveEvent extends WorkFormEvent {
		SaveEvent(WorkFormNew source, WorkNew work) {
			super(source, work);
		}
	}

	public static class DeleteEvent extends WorkFormEvent {
		DeleteEvent(WorkFormNew source, WorkNew work) {
			super(source, work);
		}
